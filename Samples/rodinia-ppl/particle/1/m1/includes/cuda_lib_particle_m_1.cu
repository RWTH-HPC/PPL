...
float powi(float x, int32_t n) {
	float res_6tMulGMdRQ;
	res_6tMulGMdRQ = 1;
	for ( int32_t i = 0; i < n; i++ ) {
		res_6tMulGMdRQ *= x;
	}
		return res_6tMulGMdRQ;
}
int32_t roundDouble(double value) {
	int32_t newValue_AS7CTY5EbR;
	newValue_AS7CTY5EbR = Cast2Int(value);
	if ((value - newValue_AS7CTY5EbR < 0.5)) {
				return newValue_AS7CTY5EbR;
	} else {
				return newValue_AS7CTY5EbR + 1;
	}
}

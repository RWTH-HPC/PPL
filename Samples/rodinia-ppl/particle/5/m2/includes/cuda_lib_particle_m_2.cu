...
float powi(float x, int32_t n) {
	float res_DBSCNyqqJY;
	res_DBSCNyqqJY = 1;
	for ( int32_t i = 0; i < n; i++ ) {
		res_DBSCNyqqJY *= x;
	}
		return res_DBSCNyqqJY;
}
int32_t roundDouble(double value) {
	int32_t newValue_XY0JU0KJE7;
	newValue_XY0JU0KJE7 = Cast2Int(value);
	if ((value - newValue_XY0JU0KJE7 < 0.5)) {
				return newValue_XY0JU0KJE7;
	} else {
				return newValue_XY0JU0KJE7 + 1;
	}
}

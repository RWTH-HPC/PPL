...
float powi(float x, int32_t n) {
	float res_oxRsv0UQ9a;
	res_oxRsv0UQ9a = 1;
	for ( int32_t i = 0; i < n; i++ ) {
		res_oxRsv0UQ9a *= x;
	}
		return res_oxRsv0UQ9a;
}
int32_t roundDouble(double value) {
	int32_t newValue_YvxfMDbPeS;
	newValue_YvxfMDbPeS = Cast2Int(value);
	if ((value - newValue_YvxfMDbPeS < 0.5)) {
				return newValue_YvxfMDbPeS;
	} else {
				return newValue_YvxfMDbPeS + 1;
	}
}

...
#include <stdio.h>
#include <stdlib.h>
#include "cuda_lib_particle_m_1.hxx"
#include "cuda_lib_particle_m_1.cuh"
#include <iostream>
#include <cuda.h>
...
float powi(float x, int32_t n) {
	float res_T9Ir5nQ2GL;
	res_T9Ir5nQ2GL = 1;
	for ( int32_t i = 0; i < n; i++ ) {
		res_T9Ir5nQ2GL *= x;
	}
		return res_T9Ir5nQ2GL;
}
int32_t roundDouble(double value) {
	int32_t newValue_2ko5Hj0BKj;
	newValue_2ko5Hj0BKj = Cast2Int(value);
	if ((value - newValue_2ko5Hj0BKj < 0.5)) {
				return newValue_2ko5Hj0BKj;
	} else {
				return newValue_2ko5Hj0BKj + 1;
	}
}

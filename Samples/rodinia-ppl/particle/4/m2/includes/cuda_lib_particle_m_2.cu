...
float powi(float x, int32_t n) {
	float res_NFXTn7BXZT;
	res_NFXTn7BXZT = 1;
	for ( int32_t i = 0; i < n; i++ ) {
		res_NFXTn7BXZT *= x;
	}
		return res_NFXTn7BXZT;
}
int32_t roundDouble(double value) {
	int32_t newValue_vdiEZXAQRx;
	newValue_vdiEZXAQRx = Cast2Int(value);
	if ((value - newValue_vdiEZXAQRx < 0.5)) {
				return newValue_vdiEZXAQRx;
	} else {
				return newValue_vdiEZXAQRx + 1;
	}
}

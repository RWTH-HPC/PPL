...
float powi(float x, int32_t n) {
	float res_ukXs7zmS4I;
	res_ukXs7zmS4I = 1;
	for ( int32_t i = 0; i < n; i++ ) {
		res_ukXs7zmS4I *= x;
	}
		return res_ukXs7zmS4I;
}
int32_t roundDouble(double value) {
	int32_t newValue_MqrQpGg7Pe;
	newValue_MqrQpGg7Pe = Cast2Int(value);
	if ((value - newValue_MqrQpGg7Pe < 0.5)) {
				return newValue_MqrQpGg7Pe;
	} else {
				return newValue_MqrQpGg7Pe + 1;
	}
}

...
float powi(float x, int32_t n) {
	float res_CajKbuXumY;
	res_CajKbuXumY = 1;
	for ( int32_t i = 0; i < n; i++ ) {
		res_CajKbuXumY *= x;
	}
		return res_CajKbuXumY;
}
int32_t roundDouble(double value) {
	int32_t newValue_Ff5WQX8lbk;
	newValue_Ff5WQX8lbk = Cast2Int(value);
	if ((value - newValue_Ff5WQX8lbk < 0.5)) {
				return newValue_Ff5WQX8lbk;
	} else {
				return newValue_Ff5WQX8lbk + 1;
	}
}

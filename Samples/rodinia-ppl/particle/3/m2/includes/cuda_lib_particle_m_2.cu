...
float powi(float x, int32_t n) {
	float res_pbnIg08nDg;
	res_pbnIg08nDg = 1;
	for ( int32_t i = 0; i < n; i++ ) {
		res_pbnIg08nDg *= x;
	}
		return res_pbnIg08nDg;
}
int32_t roundDouble(double value) {
	int32_t newValue_4GT1p8MsE3;
	newValue_4GT1p8MsE3 = Cast2Int(value);
	if ((value - newValue_4GT1p8MsE3 < 0.5)) {
				return newValue_4GT1p8MsE3;
	} else {
				return newValue_4GT1p8MsE3 + 1;
	}
}

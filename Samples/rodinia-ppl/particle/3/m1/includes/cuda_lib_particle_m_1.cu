...
float powi(float x, int32_t n) {
	float res_xBrQET9Nqs;
	res_xBrQET9Nqs = 1;
	for ( int32_t i = 0; i < n; i++ ) {
		res_xBrQET9Nqs *= x;
	}
		return res_xBrQET9Nqs;
}
int32_t roundDouble(double value) {
	int32_t newValue_v9OyzXLiJa;
	newValue_v9OyzXLiJa = Cast2Int(value);
	if ((value - newValue_v9OyzXLiJa < 0.5)) {
				return newValue_v9OyzXLiJa;
	} else {
				return newValue_v9OyzXLiJa + 1;
	}
}

...
int main(int argc, char** argv)
{
    ...
    #pragma omp parallel
    {
        ...
        {
            double* d = data + i * FEATURE_LENGTH;
            normalize(d, normalized + i * FEATURE_LENGTH, 0.4, 0.3);
	    
 	    double feature = extract(normalized + i * FEATURE_LENGTH);
	    classes[i] = classify(feature, classifier_thresh, classifier_weights);
        }
    }

...
void cuda_wrapper_increment_BlSSUcCEFx(int32_t* initial_3IA28ImCoz, int32_t* result_SbELvBVQBb ) {

	kernel_cuda_wrapper_increment_BlSSUcCEFx<<<1, 168>>> (initial_3IA28ImCoz, result_SbELvBVQBb );
}

__global__ 
void kernel_cuda_wrapper_increment_BlSSUcCEFx(int32_t*  initial_3IA28ImCoz, int32_t* result_SbELvBVQBb ) {
	...
	for ( int range_iterator = 0; range_iterator < exec_range + 0; range_iterator++) {
		int INDEX_BlSSUcCEFx = tid * exec_range + range_iterator + 100;
		if (tid < 100) {
			INDEX_BlSSUcCEFx -= 100;
		}
		result_SbELvBVQBb[(INDEX_BlSSUcCEFx)] = initial_3IA28ImCoz[(INDEX_BlSSUcCEFx)] + 1;
	}
}

...
#include <stdio.h>
#include <stdlib.h>
#include "cuda_lib_FlowCPU2ACC.hxx"
#include "cuda_lib_FlowCPU2ACC.cuh"
#include <iostream>
#include <cuda.h>
...
void cuda_wrapper_increment_A1ROJRS0uZ(int32_t* intermediate_cigTaotlR0, int32_t* result_JLXTjYpslU ) {

	kernel_cuda_wrapper_increment_A1ROJRS0uZ<<<1, 168>>> (intermediate_cigTaotlR0, result_JLXTjYpslU );
}
...
void kernel_cuda_wrapper_increment_A1ROJRS0uZ(int32_t*  intermediate_cigTaotlR0, int32_t* result_JLXTjYpslU ) {
	...
	for ( int range_iterator = 0; range_iterator < exec_range + 0; range_iterator++) {
		int INDEX_A1ROJRS0uZ = tid * exec_range + range_iterator + 32;
		if (tid < 32) {
			INDEX_A1ROJRS0uZ -= 32;
		}
		result_JLXTjYpslU[(INDEX_A1ROJRS0uZ)] = intermediate_cigTaotlR0[(INDEX_A1ROJRS0uZ)] + 1;
	}
}

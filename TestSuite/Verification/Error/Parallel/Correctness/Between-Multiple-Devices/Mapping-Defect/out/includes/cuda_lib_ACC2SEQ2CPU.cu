...
#include <stdio.h>
#include <stdlib.h>
#include "cuda_lib_ACC2SEQ2CPU.hxx"
#include "cuda_lib_ACC2SEQ2CPU.cuh"
#include <iostream>
#include <cuda.h>
...
void cuda_wrapper_increment_l4eRRHe9xh(int32_t* initial_KsBnEc0LWd, int32_t* intermediate_R0hHU019IF ) {

	kernel_cuda_wrapper_increment_l4eRRHe9xh<<<1, 168>>> (initial_KsBnEc0LWd, intermediate_R0hHU019IF );
}
...
void kernel_cuda_wrapper_increment_l4eRRHe9xh(int32_t*  initial_KsBnEc0LWd, int32_t* intermediate_R0hHU019IF ) {
	...
	for ( int range_iterator = 0; range_iterator < exec_range + 0; range_iterator++) {
		int INDEX_l4eRRHe9xh = tid * exec_range + range_iterator + 32;
		if (tid < 32) {
			INDEX_l4eRRHe9xh -= 32;
		}
		intermediate_R0hHU019IF[(INDEX_l4eRRHe9xh)] = initial_KsBnEc0LWd[(INDEX_l4eRRHe9xh)] + 1;
	}
}

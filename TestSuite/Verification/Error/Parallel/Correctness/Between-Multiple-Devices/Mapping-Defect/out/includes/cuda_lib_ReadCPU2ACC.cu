...
#include <stdio.h>
#include <stdlib.h>
#include "cuda_lib_ReadCPU2ACC.hxx"
#include "cuda_lib_ReadCPU2ACC.cuh"
#include <iostream>
#include <cuda.h>
...
void cuda_wrapper_increment_pqMP2YoX9q(int32_t* initial_Assa9vjb42, int32_t* result_8D0IqABlbo ) {

	kernel_cuda_wrapper_increment_pqMP2YoX9q<<<1, 168>>> (initial_Assa9vjb42, result_8D0IqABlbo );
}
...
void kernel_cuda_wrapper_increment_pqMP2YoX9q(int32_t*  initial_Assa9vjb42, int32_t* result_8D0IqABlbo ) {
	...
	for ( int range_iterator = 0; range_iterator < exec_range + 0; range_iterator++) {
		int INDEX_pqMP2YoX9q = tid * exec_range + range_iterator + 32;
		if (tid < 32) {
			INDEX_pqMP2YoX9q -= 32;
		}
		result_8D0IqABlbo[(INDEX_pqMP2YoX9q)] = initial_Assa9vjb42[(INDEX_pqMP2YoX9q)] + 1;
	}
}

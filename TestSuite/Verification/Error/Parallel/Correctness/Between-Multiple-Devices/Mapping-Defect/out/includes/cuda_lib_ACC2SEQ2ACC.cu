...
#include <stdio.h>
#include <stdlib.h>
#include "cuda_lib_ACC2SEQ2ACC.hxx"
#include "cuda_lib_ACC2SEQ2ACC.cuh"
#include <iostream>
#include <cuda.h>
...
void cuda_wrapper_increment_W2KH8AD4Ma(int32_t* initial_8GPdo0d1W0, int32_t* intermediate_rKNPOUzEou ) {

	kernel_cuda_wrapper_increment_W2KH8AD4Ma<<<1, 168>>> (initial_8GPdo0d1W0, intermediate_rKNPOUzEou );
}
...
void kernel_cuda_wrapper_increment_W2KH8AD4Ma(int32_t*  initial_8GPdo0d1W0, int32_t* intermediate_rKNPOUzEou ) {
	...
	for ( int range_iterator = 0; range_iterator < exec_range + 0; range_iterator++) {
		int INDEX_W2KH8AD4Ma = tid * exec_range + range_iterator + 32;
		if (tid < 32) {
			INDEX_W2KH8AD4Ma -= 32;
		}
		intermediate_rKNPOUzEou[(INDEX_W2KH8AD4Ma)] = initial_8GPdo0d1W0[(INDEX_W2KH8AD4Ma)] + 1;
	}
}

void cuda_wrapper_increment_2COEAgXFgU(int32_t* intermediate_rKNPOUzEou, int32_t* result_VJcCRyR3D8 ) {

	kernel_cuda_wrapper_increment_2COEAgXFgU<<<1, 168>>> (intermediate_rKNPOUzEou, result_VJcCRyR3D8 );
}
...
void kernel_cuda_wrapper_increment_2COEAgXFgU(int32_t*  intermediate_rKNPOUzEou, int32_t* result_VJcCRyR3D8 ) {
	...
	for ( int range_iterator = 0; range_iterator < exec_range + 0; range_iterator++) {
		int INDEX_2COEAgXFgU = tid * exec_range + range_iterator + 32;
		if (tid < 32) {
			INDEX_2COEAgXFgU -= 32;
		}
		result_VJcCRyR3D8[(INDEX_2COEAgXFgU)] = intermediate_rKNPOUzEou[(INDEX_2COEAgXFgU)] + 1;
	}
}

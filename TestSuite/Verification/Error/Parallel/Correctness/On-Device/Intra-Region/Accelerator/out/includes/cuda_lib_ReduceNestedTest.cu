...
#include <stdio.h>
#include <stdlib.h>
#include "cuda_lib_ReduceNestedTest.hxx"
#include "cuda_lib_ReduceNestedTest.cuh"
#include <iostream>
#include <cuda.h>
...
void cuda_wrapper_rowSum_JTkoi9qzzS(int32_t* initial_Cro7ZoaMZD, int32_t* result_mrCUu80Ufm ) {

	kernel_cuda_wrapper_rowSum_JTkoi9qzzS<<<1, 168>>> (initial_Cro7ZoaMZD, result_mrCUu80Ufm );
}
...
void kernel_cuda_wrapper_rowSum_JTkoi9qzzS(int32_t*  initial_Cro7ZoaMZD, int32_t* result_mrCUu80Ufm ) {
	...
	for ( int range_iterator = 0; range_iterator < exec_range + 0; range_iterator++) {
		int INDEX_JTkoi9qzzS = tid * exec_range + range_iterator + 32;
		if (tid < 32) {
			INDEX_JTkoi9qzzS -= 32;
		}
		for (size_t INDEX_c2ZvZLDi0B = 0; INDEX_c2ZvZLDi0B < 0 + 200; ++INDEX_c2ZvZLDi0B) {
			result_mrCUu80Ufm[(INDEX_JTkoi9qzzS)] += initial_Cro7ZoaMZD[200LL * (INDEX_JTkoi9qzzS) + (INDEX_c2ZvZLDi0B)] + 1;
		}
	}
}

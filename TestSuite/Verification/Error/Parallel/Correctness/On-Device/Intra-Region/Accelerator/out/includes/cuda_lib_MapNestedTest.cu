...
#include <stdio.h>
#include <stdlib.h>
#include "cuda_lib_MapNestedTest.hxx"
#include "cuda_lib_MapNestedTest.cuh"
#include <iostream>
#include <cuda.h>
...
void cuda_wrapper_matrixIncrement_FU1WgUdkbG(int32_t* initial_4p4ktWJZxo, int32_t* result_NcgNP2jlMG ) {

	kernel_cuda_wrapper_matrixIncrement_FU1WgUdkbG<<<1, 168>>> (initial_4p4ktWJZxo, result_NcgNP2jlMG );
}
...
void kernel_cuda_wrapper_matrixIncrement_FU1WgUdkbG(int32_t*  initial_4p4ktWJZxo, int32_t* result_NcgNP2jlMG ) {
	...
	for ( int range_iterator = 0; range_iterator < exec_range + 0; range_iterator++) {
		int INDEX_FU1WgUdkbG = tid * exec_range + range_iterator + 32;
		if (tid < 32) {
			INDEX_FU1WgUdkbG -= 32;
		}
		for (size_t INDEX_UcAIRrfFCA = 0; INDEX_UcAIRrfFCA < 0 + 200; ++INDEX_UcAIRrfFCA) {
			result_NcgNP2jlMG[200LL * (INDEX_FU1WgUdkbG) + (INDEX_UcAIRrfFCA)] = initial_4p4ktWJZxo[200LL * (INDEX_FU1WgUdkbG) + (INDEX_UcAIRrfFCA)] + 1;
		}
	}
}

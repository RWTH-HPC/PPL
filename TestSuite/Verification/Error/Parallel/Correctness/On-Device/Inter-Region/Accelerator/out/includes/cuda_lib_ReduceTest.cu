...
void cuda_wrapper_sum_Ji7XGfHaKX(int32_t* initial_USRQYvaGss, int32_t* temp_data_BTl8uhu8rH, pthread_mutex_t reduction_lock_BTl8uhu8rH) {
	int32_t* temp_res;
	int32_t* d_temp_res;
	temp_res = Init_List(temp_res, 1);
	cudaMalloc(&d_temp_res, 1 * sizeof(int32_t));

	kernel_cuda_wrapper_sum_Ji7XGfHaKX<<<1, 168>>> (initial_USRQYvaGss, d_temp_res );
	cudaMemcpy(&temp_res[0], &d_temp_res[0], sizeof(int32_t) * 1, cudaMemcpyDeviceToHost);
	cudaFree(d_temp_res);
	pthread_mutex_lock(&reduction_lock_BTl8uhu8rH);
	temp_data_BTl8uhu8rH[0] = reduction_sum(temp_data_BTl8uhu8rH[0], temp_res, 1, 0);
	pthread_mutex_unlock(&reduction_lock_BTl8uhu8rH);
	std::free(temp_res);
}
...
void kernel_cuda_wrapper_sum_Ji7XGfHaKX(int32_t* initial_USRQYvaGss, int32_t* result_CFeZRcqAml ) {
	...
	int id = threadIdx.x;
	__shared__ int32_t result_CFeZRcqAml_s [168];
	result_CFeZRcqAml_s[id] = 0;
	...
	for ( size_t range_iterator = 0; range_iterator < exec_range + 0; range_iterator++) {
		int INDEX_Ji7XGfHaKX = tid * exec_range + range_iterator + 100;
		if (tid < 100) {
			INDEX_Ji7XGfHaKX -= 100;
		}
		result_CFeZRcqAml_s[id] += initial_USRQYvaGss[(INDEX_Ji7XGfHaKX)];
	}
	__syncthreads();

	if (168 >= 512) { if (id < 256) { result_CFeZRcqAml_s[id] += result_CFeZRcqAml_s[id + 256]; } __syncthreads(); }
	if (168 >= 256) { if (id < 128) { result_CFeZRcqAml_s[id] += result_CFeZRcqAml_s[id + 128]; } __syncthreads(); }
	if (168 >= 128) { if (id < 64) { result_CFeZRcqAml_s[id] += result_CFeZRcqAml_s[id + 64]; } __syncthreads(); }

	if ( id < 32) {
		if (168 >= 64) {result_CFeZRcqAml_s[id] += result_CFeZRcqAml_s[id + 32]; }
		__syncwarp();
		if (168 >= 32) {result_CFeZRcqAml_s[id] += result_CFeZRcqAml_s[id + 16]; }
		__syncwarp();
		if (168 >= 16) {result_CFeZRcqAml_s[id] += result_CFeZRcqAml_s[id + 8]; }
		__syncwarp();
		if (168 >= 8) {result_CFeZRcqAml_s[id] += result_CFeZRcqAml_s[id + 4]; }
		__syncwarp();
		if (168 >= 4) {result_CFeZRcqAml_s[id] += result_CFeZRcqAml_s[id + 2]; }
		__syncwarp();
		if (168 >= 2) {result_CFeZRcqAml_s[id] += result_CFeZRcqAml_s[id + 1]; }
	}
	__syncwarp();
	if ( id == 0) {
		if ( tid == 0) {
			for (size_t i = 128; i < 168; i++) {
				result_CFeZRcqAml_s[0] += result_CFeZRcqAml_s[i];
			}
		}
		result_CFeZRcqAml[blockIdx.x] = result_CFeZRcqAml_s[0];
	}
}

void cuda_wrapper_sum_qF8o5kc6LL(int32_t* initial_USRQYvaGss, int32_t* temp_data_BTl8uhu8rH, pthread_mutex_t reduction_lock_BTl8uhu8rH) {
	int32_t* temp_res;
	int32_t* d_temp_res;
	temp_res = Init_List(temp_res, 1);
	cudaMalloc(&d_temp_res, 1 * sizeof(int32_t));

	kernel_cuda_wrapper_sum_qF8o5kc6LL<<<1, 168>>> (initial_USRQYvaGss, d_temp_res );
	cudaMemcpy(&temp_res[0], &d_temp_res[0], sizeof(int32_t) * 1, cudaMemcpyDeviceToHost);
	cudaFree(d_temp_res);
	pthread_mutex_lock(&reduction_lock_BTl8uhu8rH);
	temp_data_BTl8uhu8rH[0] = reduction_sum(temp_data_BTl8uhu8rH[0], temp_res, 1, 0);
...
void kernel_cuda_wrapper_sum_qF8o5kc6LL(int32_t* initial_USRQYvaGss, int32_t* result_CFeZRcqAml ) {
	...
	for ( size_t range_iterator = 0; range_iterator < exec_range + 0; range_iterator++) {
		int INDEX_qF8o5kc6LL = tid * exec_range + range_iterator + 100;
		if (tid < 100) {
			INDEX_qF8o5kc6LL -= 100;
		}
		result_CFeZRcqAml_s[id] += initial_USRQYvaGss[(INDEX_qF8o5kc6LL)];
	}
	__syncthreads();

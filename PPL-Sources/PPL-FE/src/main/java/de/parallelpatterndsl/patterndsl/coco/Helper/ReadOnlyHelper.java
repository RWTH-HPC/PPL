 ...
 */
public class ReadOnlyHelper implements PatternDSLVisitor {

    private boolean readOnly = true;

    private ArrayList<String> parameters;

    public ReadOnlyHelper(ArrayList<String> parameters) {
    ...
    public void visit(ASTNameExpression node) {
        for (String element: parameters) {
            if(node.getName().equals(element)) {
                Log.error(node.get_SourcePositionStart() + " Parameter changed: "+ node.getName());
                readOnly = false;
            }
        }
